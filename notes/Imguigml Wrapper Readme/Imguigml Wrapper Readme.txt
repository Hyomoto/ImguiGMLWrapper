...
objImgui.modals -> Used to open modals.
	confirm( name, text, yes, no ) -> Creates a "yes/no" style confirmation modal.


objImgui.inspector -> Used to modify the inspector.
	set_default( index, variables... ) -> Adds the default variables to the given object index.


objImgui.mouse -> Contains the mouse state against imgui inputs, will be false if imgui is using the mouse.
	left	-> returns the state of the left mouse button
	right	-> returns the state of the right mouse button


objImgui.settings -> For reading and writing savable variables.
...
objImgui.search -> A simple way to set up filterable search terms.
